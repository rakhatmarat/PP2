...
class Coin(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.weight = random.randint(1, 5)  # Random weight for the coin
...
N_COINS_FOR_SPEED_INCREASE = 5  # Number of coins required for speed increase
current_coin_count = 0  # Counter to keep track of coins collected
...
while True:
    ...
    for event in pygame.event.get():
        if event.type == INC_SPEED:
            ...
            if current_coin_count >= N_COINS_FOR_SPEED_INCREASE:
                for enemy in enemies:
                    enemy.rect.y += int(SPEED)  # Increase enemy speed
                current_coin_count = 0  # Reset coin count after speed increase
    ...
    if pygame.sprite.spritecollideany(P1, coins_group):
        COINS += 1
        current_coin_count += 1  # Increment coin count
        coins_group.remove(C1)  # Remove collected coin
        C1 = Coin()  # Generate a new coin
        coins_group.add(C1)  # Add the new coin to the group
